  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        console.error(err);
        setError("Datei konnte nicht verarbeitet werden. Ist es eine gültige PDF?");
      }
    },
    [onFileProcess, year],
  );
    ...
    <div className="w-full max-w-xl mx-auto space-y-6 animate-in">
      <div className="text-center space-y-2">
        <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary to-purple-600">
          Gottesdienstplan hochladen
        </h2>
        <p className="text-muted-foreground">
          Wähle das Kalenderjahr und ziehe die PDF hierher, um sie in einen digitalen Kalender zu
          konvertieren.
        </p>
      </div>
      ...
      >
        ...
        <div className="mt-6 space-y-1">
          <p className="font-semibold text-lg">
            {isProcessing ? "PDF wird verarbeitet …" : "PDF hier ablegen"}
          </p>
          <p className="text-sm text-muted-foreground">
            {isProcessing ? "Termine werden ausgelesen – bitte kurz warten." : "oder klicken, um eine Datei auszuwählen"}
          </p>
        </div>
      ...
      <div className="text-center text-xs text-muted-foreground/60">
        Unterstütztes Format: PDF mit Datumszeilen wie „Wochentag dd.mm.“
      </div>
    </div>
  );
}
